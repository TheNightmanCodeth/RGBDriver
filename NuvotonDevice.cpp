...
namespace Nuvoton {
    ...
    bool Device::runProgram(RGBProgram program) {
        enter(getDevicePort());
        if (program.pulse) {
            enablePulsing();
        }
    ...
    uint8_t Device::makeE4Val(RGBProgram program) {
        uint8_t pulse, flash;
